...
# This script is used to rename the checkers repo to the module name of your choice.
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    find . -not -path './.*' -type f -exec sed -i -e "s,call3hudson/checkers,$MODULE_NAME,g" {} \;
    find . -name '*.proto' -type f -exec sed -i -e "s,cosmosregistry.checkers,$(echo "$MODULE_NAME" | tr '/' '.'),g" {} \;
    find . -name 'protocgen.sh' -type f -exec sed -i -e "s,rm -rf github.com cosmosregistry,rm -rf github.com $USERNAME,g" {} \;
    find . -not -path './.*' -type f -exec sed -i -e "s,checkers,$REPO,g" {} \;
else
    find . -not -path './.*' -type f -exec sed -i '' -e "s,call3hudson/checkers,$MODULE_NAME,g" {} \;
    find . -name '*.proto' -type f -exec sed -i '' -e "s,cosmosregistry.checkers,$(echo "$MODULE_NAME" | tr '/' '.'),g" {} \;
    find . -name 'protocgen.sh' -type f -exec sed -i '' -e "s,rm -rf github.com cosmosregistry,rm -rf github.com $USERNAME,g" {} \;
...
mv proto/call3hudson/checkers/* proto/$MODULE_NAME
...
# credits
echo "# This Cosmos SDK module was generated using <https://github.com/call3hudson/checkers>" > THANKS.md
